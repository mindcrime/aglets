...
public class TestAgletThreadPool extends TestCase {
    ...
    /**
     * Test method for {@link com.ibm.aglets.thread.AgletThreadPool#pop(com.ibm.aglet.message.MessageManager)}.
     */
    public void testPop() {
	try{
		...
		assertTrue((t1 instanceof AgletThread));	// should be an aglet thread
	    }
    ...
    /**
     * Test method for {@link com.ibm.aglets.thread.AgletThreadPool#push(com.ibm.aglets.thread.AgletThread)}.
     */
    public void testPush() {
	try{
	    AgletThread t = new AgletThread(this.pool.getThreadGroup());
	    this.pool.push(t);
	    assertTrue(this.pool.contains(t));	// the pool should contain the thread now
	}catch(AgletException e){
	    throw new RuntimeException(e);
	}
