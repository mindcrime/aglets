...
public class AgletsLogger implements Cloneable{
	    ...
	    /**
	     * A map to map each class with a specific logger, thus in
	     * the future different instances of the same class can get the same
	     * logger.
	     */
	     ...
	     * Please note that this method uses a map to store the logger providden associating
	     * it with the name specified. This means that in further calls, the logger returned will
	     * be the same logger instance.
	    ...
	    */
	    ...
	    /**
	     * Logs a translating action, this works only with the debug level activated.
	     * @param src the original string
	     * @param dest the translated string
	     */
	    public void translation(String src, String dest){
		if( src != null || dest != null )
		    logger.debug("Translated/changed the string <<" + src + ">> into <<" + dest + ">>");
	    }
