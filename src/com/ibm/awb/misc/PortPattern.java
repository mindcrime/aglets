...
public class PortPattern {
	...
	private static IllegalArgumentException getException(final String msg) {
		return new IllegalArgumentException("Illegal port number : " + msg);
	}
	private static boolean isValid(final int port) {
		return port >= 0;
	}
	// for test
	...
	public PortPattern(final int port) {
	...
	public PortPattern(final String pattern) {
		if (pattern == null) {
			_type = TYPE_DEFAULT_PORT;
			return;
		}
	...
	public boolean equals(final Object obj) {
		if (obj instanceof PortPattern) {
			final PortPattern ppat = (PortPattern) obj;
	...
	public boolean equals(final PortPattern ppat) {
		if (ppat == null) {
			return false;
		}
		if (ppat._type != _type) {
	...
	public boolean isMatch(final int port) {
