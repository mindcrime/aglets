...
final public class SharedSecrets {
	...
	SharedSecret sec = this.getSharedSecret(domainName);
	...
	this._secrets.addElement(secret);
    }
	...
	while (lines.hasMoreElements()) {
		...
		if (sec != null) {
		    ...
		    SharedSecret secret = SharedSecret.convertLinesToSharedSecret(sec.elements());
	...
	Enumeration secrets = this.secrets();
	...
	SharedSecret secret = this.getSharedSecret(domainName);

	if (secret == null) {
	    throw new RuntimeException("The shared secret for '" + domainName
		    + "' does not exist.");
	}
	this._secrets.removeElement(secret);
    }
