...
public abstract class MAFAgentSystem {
	...
	synchronized private static void createResource(final String protocol) {
		...
		if (protocol_res == null) {
			try {
				final String propfile = FileUtils.getPropertyFilenameForUser(username, protocol);
	...
	private static void enableMAFURLStreamHandlers() {
		...
		try {
			final URLStreamHandlerFactory fac = new URLStreamHandlerFactory() {
				@Override
	...
	synchronized private static AgentSystemHandler getHandler(final String scheme) {
		AgentSystemHandler handler = (AgentSystemHandler) handlers.get(scheme);

		if (handler == null) {
			final String class_name = "com.ibm.maf." + scheme.toLowerCase()
			+ ".Handler";
	...
	throws MAFExtendedException {
		...
		final AgentSystemHandler handler = getHandler(protocol);

		local = l;
		handler.initMAFAgentSystem(local);
	}

	private static void initURLStreamHandlers(final String protocol) {
