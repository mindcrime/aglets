...
/**
 * This class represents a message queue. A message queue is a queue where messages are placed
 * before they can be processed. The idea is that a message manager creates its own message queue
 * and uses it to store message before it has activate a thread to process it. The use of a message queue
 * allows the uncoupling of agent threads. In fact when an agent sends a message to another agent, the sending thread
 * thru several method calls inserts the message in the message queue, and thus the sender thread returns. Thus the
 * sender thread is involved until the message has been inserted in the message queue. Once this is done, it is the addressee
 * thread in charge of processing the message itself, and this is done by the message manager that gets a thread
 * and pops a message from the message queue.
 * 
 * Please note that this class can be used as a queue for every implementation of a message, since it has been
 * generalized. Thus it can be created to manage Message or MessageImpl objects.
 * 
 * The old implementation of message queue was based on a manually set linked list between message implementations, while
 * this uses a standard linked list. Moreover this version is synchronized and thus thread safe. Finally it takes
 * care of priority, since it inserts the messages depending on their priority, thus the highest priority
 * are always at the head of the list and will be the first being popped.
 * 
...
public class MessageQueue<MSG extends Message> {
	...
	public synchronized void insertAtTop(MessageQueue<MSG> queue) {
