...
/**
 * A base window for all the windows of the Tahiti GUI. Please note that this class
 * should be used as base for other windows and that the behaviour of such windows must
 * be developed accordingly to the one expressed here. Thus, for instance, you have
 * to manage all events and to pass to this class events you don't manage.
 * 
...
public class TahitiWindow extends JFrame implements ActionListener{
	...
	/**
	 * This flag is true if this window can exit the whole application
	 * when it is closed. Set it to true when you are working with windows
	 * that should not allow users to continue with the application (e.g., login
	 * windows).
	 */
	...
	/**
	 * Default constructor, does simple layout. The layout of the window is the following:
	 * a twin panel is added to the south, with an ok/cancel button panel and a generic button
	 * panel that the user can use to add other buttons. Then there is a border layout over the whole window
	 * that can be used to display other components.
	 */
	protected TahitiWindow(boolean inizializeGUI){
	    super();
	    this.translator = AgletsTranslator.getInstance("tahiti", Locale.getDefault());
	    
	    this.setTitle(this.getClass().getName());		// translate the title to the appropriate one
	    this.windowManager = new WindowManager(this);	// add the standard management event system
	    this.addWindowListener(this.windowManager);
	    
	    if( inizializeGUI ){
        	    // set the layout to the borderlayout and add the content to the center, the
	...
	/**
	 * Adds a button to the panel button of the window using the key and the specified
	 * listener.
	 ...
	 * @param key the key for the button label and icon
	 * @param listener the listener associated to this button
	 * @param keyListener the keylistener of this button
	 * @return the button for further evaluation
	 */
	protected JButton addButton(String key, ActionListener listener, KeyListener keyListener){
	...
	/*public static void main(String argv[]){
	    TahitiWindow window = new TahitiWindow();
	    window.addButton(new JButton("ciao"));
