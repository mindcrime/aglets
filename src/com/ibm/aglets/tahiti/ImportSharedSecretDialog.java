...
public class ImportSharedSecretDialog extends TahitiDialog {
    ...
    /**
     * A static file chooser. I cache it in order to show to the user always the last
     * directory where it was working.
     */
	...
	this.fileName = JComponentBuilder.createJTextField(30, null, this.baseKey + ".fileName");
	JLabel label = JComponentBuilder.createJLabel( this.baseKey + ".fileLabel");
	panel.add(label);
	...
	if( GUICommandStrings.BROWSE_FILESYSTEM_COMMAND.equals(command)){
		...
		// now get the file
		this.fileName.setText( ((File)fileChooser.getSelectedFile()).getCanonicalPath() );
	    }catch(IOException e){
	...
	if( GUICommandStrings.OK_COMMAND.equals(command)){
		...
		JOptionPane.showMessageDialog(this,
	                      this.translator.translate(this.baseKey + ".sharedSecretExists"),
	                      this.translator.translate(this.baseKey + ".sharedSecretExists.title"),
	                      JOptionPane.ERROR_MESSAGE);
	    }
	    else{
