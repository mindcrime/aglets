...
public final class MainWindow extends TahitiWindow implements  ItemListener, ActionListener, ListSelectionListener {
	...
	/**
	 * Is the window full size or reduced?
	 */
	private boolean windowReduced = false;
	...
	MainWindow(Tahiti tahiti) {
		...
		try{
		    ...
		    String titleAddition = " " + ownerName + "@" + hosting + " - " + this.getInformativeTitle();
		    this.setTitle( this.getTitle() + titleAddition );
		}catch(Exception e){
		    logger.error("Cannot get an Aglet Runtime instance! Cannot continue!", e);
		    this.setTitle(this.getTitle()  + " " +  this.translator.translate(this.baseKey + ".title.error"));
		    return;
		}
	...
	/**
	 * Checks some run-time parameters to and returns a descriptive string to place
	 * in the window title. 
	 */
	private String getInformativeTitle() {
	    ...
	    if( brunning )
	        titleAddition = this.translator.translate(this.baseKey + ".title.running");
	    else
	        titleAddition = this.translator.translate(this.baseKey + ".title.notrunning");
	    
	    if( bsecure )
	        titleAddition += this.translator.translate(this.baseKey + ".title.secure");
	    else
	        titleAddition += this.translator.translate(this.baseKey + ".title.unsecure");
	...
	/**
	 * Returns the first (and maybe the only) selected item in the aglet list panel.
	...
	/**
	 * Manages events from the GUI within the Tahiti server.
	 * Commands issued are divided into two groups: those that require
	 * a selection in the aglet list and those that do not require it. The former
	 * kind of commands accept multiple selection, and the right action will be
	 * processed for each selected aglet. Please note that this method has been
	 * designed to reduce the code, thus the dialog for each action is associated to
	 * a global TahitiDialog instance, and then it is made modal and visible...
	 */
	public void actionPerformed(ActionEvent event) {
		...
		if( selectedProxies == null || selectedProxies.size()==0 ){
		    JOptionPane.showMessageDialog(this, this.baseKey + ".errorMessage.selectionEmpty", 
			                                this.baseKey + ".errorMessage.selectionEmpty.title",
		...
		while (iter != null && iter.hasNext()) {
			...
			dialog = new CloneAgletDialog(this, currentProxy);
	            ...
	            if( GUICommandStrings.ACTIVATE_AGLET_COMMAND.equals(command))
	        	this.activateAglet(currentProxy);
	            else
	            if( GUICommandStrings.DEACTIVATE_AGLET_COMMAND.equals(command)){
	        	// ask the user how many time it will deactivate the agent
	        	String deactivationTime = JOptionPane.showInputDialog(this,
	        		                                              this.translator.translate(this.baseKey + ".deactivationTime"),
	        		                                              this.translator.translate(this.baseKey + ".deactivationTime.title"),
	        		                                              JOptionPane.YES_NO_OPTION );
	        	try{
	        	    long millis = Long.parseLong(deactivationTime);
	        	    this.deactivateAglet(currentProxy, millis);
	        	}catch(NumberFormatException e){
	        	    this.logger.debug("Specified time for deactivation cannot be converted into a number of millis:  "+ deactivationTime);
	        	}
	            }
	            else
	            if( GUICommandStrings.MESSAGE_AGLET_COMMAND.equals(command))
	        	this.message(currentProxy, new Message("dialog"));
	            else
	            if( GUICommandStrings.DISPOSE_AGLET_COMMAND.equals(command))
	        	// dispose all the aglets
	        	dialog = new DisposeAgletDialog(this, currentProxy);
	            else
	            if( GUICommandStrings.SLEEP_AGLET_COMMAND.equals(command) ){
	        	...
	        	try{
	        	    ...
	        	    String deactivationTime = JOptionPane.showInputDialog(this,
                                    this.translator.translate(this.baseKey + ".sleepTime"),
                                    this.translator.translate(this.baseKey + ".sleepTime.title"),
                                    JOptionPane.YES_NO_OPTION );
	        	    long millis = Long.parseLong(deactivationTime);
	        	    ...
	        	    JOptionPane.showMessageDialog(this,
	        		    			  this.translator.translate(this.baseKey + ".error.sleepMessage"),
	        		    			  this.translator.translate(this.baseKey + ".error.sleepTitle"),
	        		    			  JOptionPane.ERROR_MESSAGE);
	        	}
	        	
	            }
	            else
	            if( GUICommandStrings.DISPATCH_AGLET_COMMAND.equals(command))
	        	dialog = new DispatchAgletDialog(this, currentProxy);
	            else
	            if( GUICommandStrings.INFO_AGLET_COMMAND.equals(command))
		...
		if( GUICommandStrings.CREATE_AGLET_COMMAND.equals(command))
		    dialog = new CreateAgletDialog(this);
  	        ...
  	        if( GUICommandStrings.GARBAGECOLLECTOR_COMMAND.equals(command)){
  	            this.logger.info("Forcing the garbage collector");
  	            System.gc();
  	        }
  	        else
  	        if( GUICommandStrings.REBOOT_COMMAND.equals(command)){
  	            // ask confirmation for rebooting
  	            if( JOptionPane.showConfirmDialog( this.getParent(), 
  	        	                               this.translator.translate( this.baseKey + ".rebootMessage"),
  	        	                               this.translator.translate( this.baseKey + ".rebootMessage.title"),
  	        	                               JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION )
  	        	this.reboot();
  	        }
  	        else
  	        if( GUICommandStrings.EXIT_COMMAND.equals(command)){
  	            // ask confirmation for rebooting
  	            if( JOptionPane.showConfirmDialog( this.getParent(), 
  	        	                               this.translator.translate( this.baseKey + ".shutdownMessage"),
  	        	                               this.translator.translate( this.baseKey + ".shutdownMessage.title"),
  	        	                               JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION )

  	  	            this.shutdown();
  	        }
  	        else
  	        if( GUICommandStrings.RETRACT_AGLET_COMMAND.equals(command))
  	            dialog = new RetractAgletDialog(this);
  	        else
  	        if( GUICommandStrings.REDUCE_COMMAND.equals(command)){
  	        ...
  	        if( GUICommandStrings.ENLARGE_COMMAND.equals(command)){
  	        ...
  	        if( GUICommandStrings.WEB_COMMAND.equals(command)){
  	            // get the URL of the site
  	          String webPage = this.translator.translate( this.baseKey + ".webSite");
  	          try{
  	        ...
  	        if( GUICommandStrings.MEMORY_COMMAND.equals(command))
  	        ...
  	        if( GUICommandStrings.GARBAGECOLLECTOR_COMMAND.equals(command)){
  	            ...
  	            JOptionPane.showMessageDialog(this, 
  	        	                          this.translator.translate( this.baseKey + ".garbageCollector"),
  	        	                          this.translator.translate( this.baseKey + ".garbageCollector.title"),
  	        	                          JOptionPane.INFORMATION_MESSAGE,
  	        	                          JComponentBuilder.getIcon( this.baseKey + ".garbageCollector.icon"));
  	        }
  	        else
  	        if( GUICommandStrings.THREAD_COMMAND.equals(command) )
  	            // dump the threads
	...
	/**
	 * This is an utility method that return true if the command requires
	 * an aglet to be selected in the aglet list panel.
	 * @param command the command to check
	 * @return true if the command requires that at least one aglet has been selected
	 */
	...
	public class TahitiEventHandler implements Runnable {
		...
		private void perform() throws Exception {
			...
			case CREATE:
				...
				AgletProxy proxy = Tahiti.CONTEXT.createAglet(url, _name, null);
			...
			case DISPATCH:
				...
				break;
			case RETRACT:
			    	logger.debug("Getting the remote agent back " + _proxy);
	 ...
	 * @param p the proxy of the agent to activate
	 */
	 ...
	 * @param documentRoot the documentation file to show
	 */
	...
	/**
	 * Show the web site.
	 * @param webSite the url of the web site to show.
	 */
	 ...
	 * @param p the proxy of the agent to clone
	 */
	...
	/**
	 * Creates a new agent.
	 * @param codebase the codebase of the class to create
	 * @param name the name of the agent
	 * @param reload reload it or get from the cache
	 */
	 ...
	 * @param p the proxy of the agent to deactivate
	 * @param time the millisecs for the deactivation
	 */
	 ...
	 * @param proxy the addressee agent
	 * @deprecated
	 */
	@Deprecated
	 ...
	 * @param proxy the proxy of the agent that must receive the message
	 ...
	 * @param p the proxy of the agent to dispatch
	 * @param dest the destination to which dispatch to
	 */
	...
	/**
	 * Disposes the agent.
	 * @param p the proxy of the agent to dispose
	 */
...
/*	synchronized void insertProxyToList(AgletProxy proxy) {
	...
	*/
	...
	/**
	 * Manages selections on the agent list. For now it
	 * only manages the selection/deselction of agents in order
	 * to enable or disable certain elements on the GUI (buttons and menu entries).
	 */
	...
	/**
	 * Saves the size of the window so that it can be restored
	 * once the window is restarted.
	 */
	...
	/**
	 * Dumps all the thread groups. Invokes the dumpThreadGroup method for dumping recursively
	 * each thread group.
	 *
	 */
	...
	/**
	 * Dumps each thread group, showing them with a nesting that depends from the group level.
	 * @param currentGroup the group to dump
	 * @param level the level for the nesting
	 * @param out the stream to which write the thread group
	 */
	private static void dumpThreadGroup(ThreadGroup currentGroup, int level, PrintStream out) {
	 ...
	 * and buttons to better guide the user thru the action it can take in a specific
	 * moment.
	 */
