...
public class IconRepository {
    ...
    /**
     * Stores the icons once they are loaded, the icons are accessible by symbolic names such
     * as "ok", "cancel", "create", and so on.
     */
     ...
     * @param fileName the file that contains the icon
     * @param key the symbolic name of the icon
    ...
    /**
     * A method to get an icon. The method searches in the hash if the icon exists, if so it is returned, otherwise
     * it is loaded from a file.
     * @param iconName the name of the icon requested
    ...
    /**
     * Load all icons in the property file. The method takes the name of a property file that contains
     * a few lines like key=filename and loads all the icons.
	...
	String iconPath = System.getProperty("aglets.icons.path",null);
        ...
        try{
	        ...
	        System.err.println("Exception during icon loading from file "+propFile);
	        ex.printStackTrace();
	    }
    }
    ...
    public static void defaultInit(){
        loadIconFromPropertyFile(System.getProperty("aglets.home"+"/cnf/icons.prop"));
    }
