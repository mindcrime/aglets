...
java.io.Serializable {
	 ...
	 */
	public MessageManagerImpl(final LocalAgletRef ref) {
		final HashMap<String, Integer> clone = (HashMap<String, Integer>) defaultPriorityTable.clone();
		priorityTable = clone;
		this.ref = ref;
	}
	...
	void cancelOwnerMessage() {
		if ((owner != null) && (isOwner() == false)) {
			owner.cancel("handler destroyed : message = "
					+ owner.toString());
			owner.destroy();
			owner = null;
		}
	...
	boolean isOwner() {
		if ((owner != null)
				&& owner.thread.equals(Thread.currentThread())) {
			return true;
		} else {
			return false;
		}
	...
	AgletThread popThread() throws AgletException {
		...
		if (threadPool == null)
			threadPool = AgletThreadPool.getInstance();

		// get a new thread from the pool and return it
		return threadPool.pop(this);

	}

	void postMessage(final MessageImpl msg) {
	 ...
	 * @param oneway
	 */
	private void postMessage(final MessageImpl msg, final boolean oneway) {
		// check the argument
		if (msg == null)
			return;
		...
		synchronized (message_queue) {
			if (isDestroyed()) {
				...
				if ((activationTable != null)
					...
					try {
						...
						ref.activate(); // activate the aglet ref and the
						// message manager
						msg.enableDelegation(); // set the message to accept
						// delegation
						ref.delegateMessage(msg); // delegate the message
			...
			final String msgKind = msg.getKind();
			if (msgKind != null) {
				if (priorityTable.containsKey(msgKind))
					priority = priorityTable.get(msgKind);
				else {
					priority = msg.getPriority();
					// store the priority for this kind of message, thus it can
					// be re-processed later at the same priority
					priorityTable.put(msgKind, priority);
				}
			} else
			...
			if (priority <= Message.UNQUEUED_PRIORITY) {
				...
				if (isOwner() && (oneway == false)) {
					...
					msg.setThread(owner.getThread()); // copy the thread,
					// thus if a thread
					// has already been
					// assigned we don't
					// need to get one
					...
					// will not
					// respond to itself!
					final AgletThread aThread = owner.getThread();
					if (aThread != null)
	...
	private void processNextMessage() {
		...
		if ((!isRunning()) || isSleeping() || isDestroyed()) {
	...
	void pushMessage(final MessageImpl msg) {
	 ...
	 * error).
	 */
	void pushThread(final AgletThread thread) {
		try {
			// check the argument
			if (thread == null)
				return;
	...
	public void pushThreadAndExitMonitorIfOwner(final AgletThread thread) {
		synchronized (message_queue) {
	...
	void setAgletRef(final LocalAgletRef r) {
		ref = r;
	}

	@Override
	public void setPriority(final String kind, int priority) {
	 ...
	 *            the sleeping to set
	 */
	protected synchronized void setSleeping(final boolean sleeping) {
		logger.debug("The message manager is going to sleep " + sleeping);
		this.sleeping = sleeping;
	}

	/* synchronized (message_queue) */
	void setState(final int next) {
		switch (state) {
			case UNINITIALIZED:
		...
		throw new IllegalArgumentException("Cannot proceed from "
				+ state_string[state] + " to " + state_string[next]);
	}
	...
	public String toString() {
		...
		buffer.append("\n");
		buffer.append(message_queue.toString());
		buffer.append(waiting_queue.toString());
