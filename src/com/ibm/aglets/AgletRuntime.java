...
final public class AgletRuntime extends com.ibm.aglet.system.AgletRuntime {
	 ...
	 * Certificate is read from the keystore located by "aglets.keystore.file"
	 * property.
	 * @param username an certificate alias in the keystore.
	...
	public static java.security.PrivateKey getPrivateKey(Certificate cert, 
		...
		try {
			return (java.security
				.PrivateKey)_keyStore
					.getKey(getCertificateAlias(cert), passwd);
		} catch (Exception ex) {
			ex.printStackTrace();
			return null;
		} 
	...
	/**
	 * Returns certificate of a registered user. If the user's certificate is not found,
	 * returns null.
	 * Certificate is read from the keystore located by "aglets.keystore.file"
	 * property.
	 * @param username an certificate alias in the keystore.
	...
	synchronized protected void initialize(String args[]) {
		...
		// Load the keystore.
		String keyStoreFile = System.getProperty("aglets.keystore.file", 
												 null);
		...
		String keyStorePwd = System.getProperty("aglets.keystore.password", 
												null);
		...
		try {
			...
			String anonName = 
				System.getProperty("aglets.keystore.anonymousAlias", 
								   "anonymous");
		...
		// Read policy file
		com.ibm.aglets.security.PolicyImpl policyImpl = 
			new com.ibm.aglets.security.PolicyImpl();
	...
	public void killAglet(AgletProxy proxy) throws InvalidAgletException {
		LocalAgletRef ref = 
			LocalAgletRef.getAgletRef(MAFUtil.toName(proxy.getAgletID(), 
				null));
	...
	static Name newName(Certificate authority) {
		...
		try {
			return new Name(authority.getEncoded(), b, 
	 ...
	 */
	static public void setDefaultResourceManagerFactory(ResourceManagerFactory factory) {
		check();
	...
	private void startup() {
		...
		res.setResource("aglets.stream.version", 
						String.valueOf(AgletRuntime.AGLET_STREAM_VERSION));
		res.setOptionResource("aglets.copyright", 
		...
		// 
		// 
		String export_path[] = res.getStringArray("aglets.export.path", 
												  File.pathSeparator + ", ");
		...
		if (export_path.length > 1) {
			System.out
				.println(bundle
					.getString("aglets.error.export_morethan_one"));
		} 
		...
		String default_resources[][] = {
			...
			// {"aglets.box.userid",	mailaddress},
			{
				...
				+ "examples.http.WebServerAglet " + "examples.talk.TalkMaster"
			}, 
		};
