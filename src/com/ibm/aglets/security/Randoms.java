...
public class Randoms extends Object {
	...
	public static byte[] getPseudoSeed(int length) {
		...
		Object o[] = new Object[(int)(System.currentTimeMillis() & 0xFF)];

		for (int i = 0; i < length; i++) {
			b[i] = (byte)(System.currentTimeMillis() * 3);

			long h = (new Object().hashCode() + System.currentTimeMillis()) 
					 & 0xFF;

			try {
				Thread.currentThread().sleep(h);
	 ...
	 * @param seq a byte sequence to which random number is copied
	 */
	 ...
	 * creates it and stores it into the table.
	 * @param length length of byte sequence to be generated
	 * @param useSecureRandomSeed use a secure random seed if true,
	...
	private static byte[] getSeed(int length) {
		return getSeed(length, _useSecureRandomSeed);
	}
	 ...
	 * @return use a secure random seed if true,
	 * or use an unsecure random seed otherwise.
	 */
