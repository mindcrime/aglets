...
public class Randoms extends Object {
	 ...
	 */
	public static byte[] getPseudoSeed(final int length) {
		final byte b[] = new byte[length];
		for (int i = 0; i < length; i++) {
			b[i] = (byte) (System.currentTimeMillis() * 3);

			final long h = (new Object().hashCode() + System.currentTimeMillis()) & 0xFF;
	 ...
	 */
	public static synchronized void getRandom(final int length, final byte[] seq) {
		final SecureRandom random = getRandomGenerator(length);
	 ...
	 */
	public static synchronized SecureRandom getRandomGenerator(final int length) {
	...
	public static synchronized SecureRandom getRandomGenerator(
	                                                           final int length,
	                                                           final boolean useSecureRandomSeed) {
		...
		final Object obj = table.get(len);
	...
	private static byte[] getSeed(final int length, final boolean secure) {
		if (!secure) {
			...
			} catch (final java.security.NoSuchAlgorithmException nsae) {
	 ...
	 *            seed otherwise.
	 */
	public static void setUseSecureRandomSeed(final boolean useSecureRandomSeed) {
