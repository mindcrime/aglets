...
/**
 * The <tt>PolicyDB</tt> class accesses Java policy database file
...
public class PolicyDB {
	...
	private static final String getHostPart(String name) {
	 ...
	 * @return permissions for indicated code source.
	...
	public Permissions getPermissions(CodeSource cs, boolean convert) {
		...
		for (int i = 0; i < num; i++) {
			...
			if (perms != null) {
				...
				// Exception ex = new Exception("======== TRACE ========");
				// ex.printStackTrace();
				if (permissions == null) {
									...
									Permission p = 
										...
										public Object run() {
											if (PROTOCOL_FILE
												...
												String file = 
													URIPattern
														.canonicalFilename(cb
															.getFile());

												file += File.separator 
														+ WILDCARD_ANYDIR;
												return new FilePermission(file, filep
													.getActions());
											} else if (cb
		.equals(_system_codebase)) {
	...
	private static final String getPortPart(String name) {
