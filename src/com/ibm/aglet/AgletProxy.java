...
public interface AgletProxy {
    ...
    /**
     * Activates the aglet. This is a forced activation of a deactivated/resumed
     * aglet.
     * 
    ...
    /**
     * Clones the aglet and its proxy. Note that the cloned aglet will get
     * activated. If you like to get cloned aglet which is not activated, throw
    ...
    /**
     * Deactivates the aglet. The system may store the aglet in the spool (disk
     * or memory depending on the server). The aglet will be re-activated later
     * (at the given time or manually).
     * 
     * @param duration
     *            duration of the aglet deactivating in milliseconds. If this is
     *            0, it will be activeted at the next startup time.
     * @exception InvalidAgletException
     *                if the aglet is not valid.
     ...
     * passed as an argument to <tt>Aglet#handleMessage</tt>. The delegated
     * message is sent to the receiver and will be handled in the
     * <tt>Aglet#handleMessage(Message)</tt> method.
     * 
     * @param msg
     ...
     *                if the remote server refused the dispatch request.
     * @exception InvalidAgletException
     *                if the aglet is not valid. abstract public AgletProxy
     *                dispatch(String address) throws IOException,
     *                AgletException;
     */
     ...
     * 
     * @param type
     *            an integer value specifying the aglet's state
     * @return true if the aglet is in the same state as the type give as an
     *         argument.
     * @see Aglet#ACTIVE
     * @see Aglet#INACTIVE
     */
    ...
    /**
     * Checks if the aglet proxy is invalid or not. The aglet proxy become
     * invalid in the following way.
     * <ol>
     ...
     * <p>
     * This is almost like <tt>deactivate(long duration)</tt>, but there are
     * some differences.
     * <ol>
     ...
     * </ol>
     * The aglet is re-activated by <tt>activate()</tt>. The caller is required
     * to have "deactivate" permissoin.
     * 
     * @param duration
     *            duration of the aglet deactivating in milliseconds. If this is
     *            0, it will be activeted at the next startup time.
     * @exception AgletException
     *                if the aglet cannot be suspended.
     * @exception IllegalArgumentException
     *                if the minutes parameter is negative.
     */
