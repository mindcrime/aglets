...
public interface AgletContext {
    ...
    /**
     * Adds the specified context listener to receive context events from this
     * context.
     * 
     * @param listener
     ...
     * 
     * @param codebase
     *            the codebase of the cache to clean up. if null, entire cache
     *            in this context is cleared.
     */
    abstract public void clearCache(URL codebase);

    /**
     * Creates an instance of the specified aglet class. The aglet's class code
     * file can be located on the local file system as well as on a remote
     * server. If the <tt>codeBase</tt> is <tt>null</tt>, the context will
     * search for the code in the local system's aglet search path (
     * <tt>AGLET_PATH</tt>). The createAglet method takes three arguments:
     * <tt>codeBase</tt>, <tt>code</tt>, and <tt>init</tt>:
     * 
     * @param codeBase
     *            <tt>codeBase</tt> specifies the base URL of the aglet class
     *            file, in other words, the (possibly remote) directory that
     *            contains the aglet's code. If this argument is <tt>null</tt>,
     *            then the directories specified in the local host's aglet
     *            search path are searched. The aglet search path works in a
     *            similar way to Java's class path. It is typically an
     *            environment variable that specifies a list of directories to
     *            be searched for aglet code files.
     * @param code
     *            <tt>code</tt> gives the name of the file that contains the
     *            aglet's compiled class code. This file is relative to the base
     *            URL of the aglet, and cannot be absolute.
     * @param init
     ...
     * @see Aglet#INACTIVE
     * @param type
     *            the type of aglets. ACTIVE, INACTIVE or ACTIVE | INACTIVE,
     * @return a list of proxies that matches the given state.
     */
    abstract public Enumeration getAgletProxies(int type);

    /**
     * Gets a proxy for an aglet in the current context. The selected aglet is
     * specified by its identity.
     * 
     * @param id
     ...
     *            the address specifing a remote context.
     * @param id
     *            the identity of the aglet.
     * @return the proxy.
     * @see getName
     ...
     * 
     * @param audio
     *            an absolute URL giving the location of the audio file.
     ...
     * 
     * @return the URL of the daemon. <tt>null</tt> if the hosting information
     *         is not available.
     */
     ...
     * 
     * @param image
     *            an absolute URL giving the location of the image file.
    ...
    /**
     * Gets the name of the context. Each context running in the same server can
     * be distinguished by the name. The example of an address for contexts is,
     * "atp://host.com:4434/name".
     * 
     ...
     * @exception AgletNotFoundException
     *                if the aglet could not be found.
     * 
     * @deprecated
     */
    @Deprecated
    abstract public AgletProxy retractAglet(URL url) throws IOException,
	    AgletException;
     ...
     * object is delivered to all aglets in the context before all aglets are
     ...
     * 
     * @param reactivate
     *            if false, it does not activate aglets in the deactivation
     *            spool
     * @exception SecurityException
     */
