...
/**
 * This class represents the base class for more complex itinerary. It provides the storing mechanism for locations
 * and provides method implementations. Extends this class and override apt methods to produce the itinerary logic
 * you want. Please note that this class does not provide synchronized methods, since a single itinerary object
 * is not supposed to be shared between several concurrent entities. If you need to share the itinerary you should 
 * provide synchronization by your own or use the syncrhonized itinerary facilities.
 * <BR>
 * <B>Attention:</b> this itinerary does not allow duplicated destination, that means if you try to
 * insert more than once the same destination, only the first insert will be successful.
...
public class BaseItinerary implements SimpleItinerary {
	...
	/**
	 * The agent owning this itinerary. An agent is owning this itinerary if it will use the itinerary. The
	 * itinerary will act on the owning agent.
	 */
	protected Aglet agletOwner = null;
	
	/**
	 * A vector used to store information about each destination of this itinerary.
	 */
	protected Vector destinations = null;
	
	/**
	 * This flag indicates if the itinerary is mutable or not, that means if add/delete operation will affect
	 * this itinerary or not.
	 */
	...
	/**
	 * Default constructor for this itinerary. It requires the agent that is owning this itinerary
	 * in order to know which agent needs to be moved once the itinerary methods are called.
	 * @param owner the agent owner of this itinerary, that is the agent is creating and using this itinerary.
	 * @param baseSize the initial dimension of the itinerary list, used to create the vector with the appropriate
	 * size. If an invalid size is provided (i.e., baseSize <= 0), the standard value of 10 is adopted.
	 ...
	 * @param owner the aglet that owns (i.e., will use) this itinerary.
	 * @throws IllegalArgumentException if the owner is null.
	 */
	public BaseItinerary(Aglet owner)
	throws IllegalArgumentException
	{
		this(owner,10);
	}
	 ...
	 * @param oldItinerary the itinerary to use as base for this one.
	 * @param owner the agent that must own this itinerary.
	 */
	...
	/**
	 * Adds a new destination at the end of the destination list of this itinerary. The destination is not added
	 * in the case it is null or a void string.
	 * @param destinationURI the destination to add.
	...
	/**
	 * Adds the specified destination at the specified position in the destination list.
	 * <B>ATTENTION: since this itinerary implementation is based on a Vector, inserting the 
	 * destination at the specified position could raise an ArrayIndexOutOfBound exception, since the position
	 * could exceed the vector size.</b> To avoid this, there is a check on the position to avoid inserting 
	 * in position less than zero or greater than the size of vector.
	 * @param destinationURI the destination to add.
	 * @param index the index (position) where the destinationURI should be inserted.
	 * @return true if the destination is added, false otherwise
	 */
	...
	/**
	 * Adds a destination just after the previous one as specified. If the previous destination is not contained
	 * in the itinerary, it is first added as the last one, and then the following destination is added.
	 * @param destinationURI the destination to add.
	 * #param previousDestinationURI the destination that must be visited before the one is going to be added.
	 * @return true if the destination is inserted, false otherwise
	 */
	public boolean addNextDestination(String destinationURI, String previousDestinationURI){
		// check params
		if( destinationURI         == null || destinationURI.equals("") ||
			previousDestinationURI == null || previousDestinationURI.equals("") ){
		...
		// if here the previous destination has been added, thus I can insert the 
	...
	/**
	 * Removes the destination at the specified index. The index must be valid, that is greater than 0 and less than
	 * the max size of the destination lists.
	 * @param the index of the destination to be removed.
	 * @return true if the destination has been removed, false otherwise.
	 */
	 ...
	 * @return the iterator with the list of destinations stored, at the moment, in the itinerary.
	 */
	...
	/**
	 * This method sets the itinerary as frozen, that means no more add/delete operation can be performed on this
	 * itinerary (if you want to modify the itinerary you have to create a new one). This method is useful when there's
	 * the risk that the itinerary is changed multiple times (e.g., due to a message handling). 
	 */
	 ...
	 * @return true if the itinerary is mutable (add/delete operations are allowed), false otherwise.
	 */
	 ...
	 * @param the first destination stored in this itinerary.
	 */
	...
	/**
	 * Returns the current location of the itinerary, that means the location the agent should be
	 * depending on the use of the itinerary.
	...
	/**
	 * Forces the migration to the next destination. If the migration is not possible, the stacktrace and the exception are
	 * printed, but the agent is not required to handle the exception.
	 * <B>Please note that once the agent has visited all the destinations, the execution of this method will
	 * have no effect.</B>
	 */
	public void goToNextDestination(){
		...
		try{
			...
			System.err.println("[BaseItinerary.goToNextDestination] - Exception while migrating to "+nextDest);
			e.printStackTrace();
		}
		finally{
			this.currentIndex++;
